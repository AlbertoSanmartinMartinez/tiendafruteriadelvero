...
class Correos extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->ps_versions_compliancy = array('min' => '1.6.0.4', 'max' => '1.6.3.12');
    }
    ...
    public function uninstall()
    {
        ...
        Configuration::deleteByName('CORREOS_ORDER_STATE_RETURN_ID');
        Configuration::deleteByName('CORREOS_CLIENT_NUMBER');
        Configuration::deleteByName('CORREOS_CONTRACT_NUMBER');
        Configuration::deleteByName('CORREOS_KEY');
        Configuration::deleteByName('CORREOS_ORDER_STATES');
        Configuration::deleteByName('CORREOS_PASSWORD');
        Configuration::deleteByName('CORREOS_SENDERS');
        Configuration::deleteByName('CORREOS_SHOW_CONFIG');
        Configuration::deleteByName('CORREOS_TIMESELECT_CARRIER_ID');
        Configuration::deleteByName('CORREOS_USER');
        Configuration::deleteByName('CORREOS_VERSION');
        Configuration::deleteByName('CORREOS_BANK_ACCOUNT_NUMBER');
        Configuration::deleteByName('CORREOS_CUSTOMS_ZONE');
        Configuration::deleteByName('CORREOS_MAIL_COLLECTION_CC');
        Configuration::deleteByName('CORREOS_S0236_ENABLETIMESELECT');
        Configuration::deleteByName('CORREOS_CUSTOMS_DEFAULT_CATEGORY');
      // Uninstall SQL
        $sql = array();
    ...
    public function hookHeader($params)
    {
        ...
        if (in_array($file, array('order-opc', 'order', 'orderopc', 'history', 'supercheckout'))) {
            ...
            $this->context->controller->addJS($this->_path.'views/js/front/correos_v406.js');
    ...
    public function hookExtraCarrier($params)
    {
        ...
        if (isset($cart->id_carrier)) {
            $request = Db::getInstance()->getValue(
                ...
                WHERE `type` = 'quote' AND `id_cart` = ".(int)$cart->id." 
                AND id_carrier = ".(int) $cart->id_carrier." 
                ORDER BY date DESC"
            );
            if (_PS_MAGIC_QUOTES_GPC_) {
                $request = str_replace("u00", "\u00", $request);
            }
            $request_data = (array)Tools::jsonDecode($request);
        }
